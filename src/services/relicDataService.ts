...
import { loadRelicsData as loadStaticRelicsData } from './staticDataService';
...
// Load relic data from centralized service
const loadRelicsData = async (): Promise<any[]> => {
  try {
    return await loadStaticRelicsData();
  } catch (error) {
    console.error('Failed to load relics data from static service:', error);
    throw new Error('Failed to load relic drop data from static service.');
  }
};
...
const adjustDropChances = (drops: RelicRewardItem[], refinementLevel: VoidRelic['rarity']): RelicRewardItem[] => {
  ...
  return drops.map(drop => {
    const rarity = drop.rarity as 'Common' | 'Uncommon' | 'Rare';
    return {
      ...drop,
      dropChance: (rarity && dropChances[rarity]) || drop.dropChance // Use hardcoded chance or fallback to original
    };
  });
};
...
export const getRelicDropsByName = async (relicName: string, rarity: VoidRelic['rarity'] = 'intact'): Promise<RelicRewardItem[] | undefined> => {
  try {
    ...
    if (!relic) {
      console.warn(`No relic found for: ${relicName} (base: ${baseRelicName})`);
      return undefined;
    }

    if (!relic.rewards) {
      console.warn(`Relic found but no rewards data: ${relicName}`);
      return undefined;
    }
...
export const analyzeOptimalRefinementStrategy = async (
  ...
  try {
    ...
    , 'intact' as VoidRelic['rarity']) || 'intact';
    ...
    const optimalExpectedValue = expectedValues[optimalRefinementLevel];
