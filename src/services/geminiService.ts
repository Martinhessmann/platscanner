...
const parseDetectedItems = (responseText: string): DetectedItem[] => {
  ...
  lines.forEach((line, index) => {
    ...
    const syndicateMatch = line.match(/^(.*?)\s*\|\s*([\d,]+)/);
    if (syndicateMatch) {
      const name = syndicateMatch[1].trim();
      const standingStr = syndicateMatch[2].replace(/,/g, '');
...
const analyzeSyndicate = async (imageBase64: string, mimeType: string): Promise<string> => {
  const syndicatePrompt = `Analyze this Warframe Syndicate Offerings screen. Extract only tradable rewards (weapons, augment mods, blueprints) that are visible in the screenshot and their Standing costs.

STRICT RULES:
- Read the ACTUAL image. Do not invent items.
- Prefer items likely tradable on Warframe Market (e.g., Telos/Secura/Synoid/Rakta/Sancti weapons, augment mods). You may include items if not sure.
- Ignore pure sigils, simulacrum access, caches, or consumable blueprints unless clearly visible and tradable.
- Use the standing price numbers shown on each card (e.g., 25,000 or 100,000). Remove commas in your output parsing is allowed.

RESPONSE FORMAT:
Return each item on its own line using exactly this format:
ITEM_NAME | STANDING
...
If you cannot clearly see any items, respond with "NONE_DETECTED".`;
